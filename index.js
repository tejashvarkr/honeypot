...
app.post('/api/honeypot/interact', authenticateRequest, async (req, res) => {
  ...
  try {
    if (!sessionId || !message || !message.text) {
      return res.status(400).json({ status: "error", message: "Missing sessionId or message text." });
    }
    ...
    // Strict prompt to ensure valid JSON and brevity
    const systemPrompt = `
      Roleplay as a ${targetRole}. Personality: ${targetPersonality}. Vulnerability: ${targetVulnerability}.
      Goal: Waste the scammer's time and bait them into revealing UPI IDs, links, or phone numbers.
      
      RULES:
      1. Your "reply" back to the scammer MUST be under 150 characters.
    ...
    const rawResultText = aiResponse.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!rawResultText) throw new Error("Empty AI response");

    // --- ROBUST JSON EXTRACTION ---
    let result;
    try {
      // Find the first '{' and last '}' to isolate JSON if the AI included text around it
      const start = rawResultText.indexOf('{');
      const end = rawResultText.lastIndexOf('}');
      if (start === -1 || end === -1) throw new Error("No JSON structure found");
      
      const jsonString = rawResultText.substring(start, end + 1);
      result = JSON.parse(jsonString);
    } catch (parseError) {
      console.error("Critical Parse Error. Raw Text:", rawResultText);
      // Fail-safe: Use regex intel and a generic reply if AI fails
      result = { 
        reply: "I... I don't understand. What do I need to do to fix this?", 
        extracted: { bankAccounts: [], upiIds: [], phishingLinks: [], phoneNumbers: [] } 
      };
    }
    ...
    const foundNew = Object.values(regexIntel).some(arr => arr.length > 0);
    if (foundNew) {
      reportToGuvi(sessionId, session.intel, session.count, `Engaging on ${metadata.channel || 'SMS'}`);
    }
    ...
    console.error("SERVER ERROR:", err);
    res.status(500).json({ status: "error", message: "Honeypot Logic Error" });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Honeypot Engine is active on port ${PORT}`);
});
